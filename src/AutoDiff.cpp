 ...
 * @param leaf 是否为叶子节点
 */
AutoDiff::Node::Node(size_t id, std::unique_ptr<Tensor> t, bool req_grad,
                     bool leaf)
    : tensor_id(id), tensor(std::move(t)), requires_grad(req_grad),
      is_leaf(leaf) {
  ...
  debug_print_state("make_leaf开始前");
  {
    ...
    if (id_to_node.find(id) != id_to_node.end()) {
      ...
      if (requires_grad && !id_to_node[id]->grad) {
        id_to_node[id]->grad = std::make_unique<Tensor>(ShapeTag{}, t.sizes(),
                                                        t.dtype(), t.device());
...
// ======================= defer_record函数 =======================
void AutoDiff::defer_record(size_t output_id, op operation,
...
void AutoDiff::commit_record(Tensor &output) {
  ...
  if (should_create_node) {
    ...
    auto output_node = std::make_unique<Node>(output_id, std::move(tensor_ptr),
                                              requires_grad, false);
    // 确保操作类型被正确设置
...
void AutoDiff::update_requires_grad(Tensor &t, bool requires_grad) {
  ...
  // 使用更短的锁作用域
  {
    ...
    if (it != id_to_node.end() && it->second) {
      ...
      if (requires_grad) {
        if (!it->second->grad) {
          it->second->grad = std::make_unique<Tensor>(ShapeTag{}, t.sizes(),
                                                      t.dtype(), t.device());
          it->second->grad->zero();
        }
      }
 ...
 */
Tensor
AutoDiff::check_and_adjust_grad_shape(const Tensor &grad,
 ...
 * @param result 拓扑序列结果
 */
void AutoDiff::dfs_topological_sort(Node *node,
...
void AutoDiff::backward(Tensor &root, Tensor grad_output) {
...

  {
    ...
    // 设置根节点梯度，确保形状匹配
    if (!root_node->grad) {
      root_node->grad = std::make_unique<Tensor>(ShapeTag{}, root_shape,
                                                 root.dtype(), root.device());
    }

    // 检查并调整梯度输出形状，确保与根节点形状匹配
    Tensor adjusted_grad_output =
        check_and_adjust_grad_shape(grad_output, root_shape);
      ...
      if (node->grad) {
        ...
        for (size_t input_id : node->input_ids) {
          ...
          if (input_it != id_to_node.end() && input_it->second &&
            ...
            if (node->operation == op::Add) {
              ...
              if (input_id == node->input_ids[0]) {
                ...
                if (a_it != id_to_node.end() && a_it->second &&
                  ...
                  input_grad_val =
                      -a_tensor * node_grad / (b_tensor * b_tensor);
                }
              }
              ...
              // 这里使用 node->tensor（softmax 输出 y）而不是重新计算
              // softmax(x) node->op_param_i 保存 dim（由 Tensor::softmax(dim)
              // 记录）
              ...
              if (shape.size() == 1) {
                ...
                Tensor dx(ShapeTag{}, shape, y_tensor.dtype(),
                          y_tensor.device());
              ...
              // ∂CrossEntropy(pred, target)/∂pred = (softmax(pred) - target) *
              // grad 找到另一个输入张量（目标张量）
              size_t target_id = (input_id == node->input_ids[0])
                                     ? node->input_ids[1]
                                     : node->input_ids[0];
              ...
              if (target_it != id_to_node.end() && target_it->second) {
                ...
                if (pred_shape.size() == 2) {
                  ...
                  Tensor softmax_pred(ShapeTag{}, pred_shape,
                                      pred_tensor.dtype(),
                                      pred_tensor.device());
                  ...
                  for (size_t i = 0; i < batch_size; ++i) {
                    ...
                    for (size_t j = 0; j < num_classes; ++j) {
                      exp_sum +=
                          std::exp(pred_data[i * num_classes + j] - max_val);
                    }
                  ...
                  for (size_t j = 0; j < num_classes; ++j) {
                    softmax_data[j] =
                        std::exp(pred_data[j] - max_val) / exp_sum;
                  }
            ...
            Tensor adjusted_grad_val = check_and_adjust_grad_shape(
                input_grad_val, input_node.tensor->sizes());
...
Tensor create_empty_tensor() {
  return Tensor(ShapeTag{}, std::vector<size_t>{}, DType::kFloat,
                DeviceType::kCPU);
}
