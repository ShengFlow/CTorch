...
public:
    ...
    void make_leaf(Tensor& t, bool requires_grad) {
        if (tensor_to_node.find(&t) != tensor_to_node.end()) return;
    ...
    void record_op(Tensor& result, op operation, std::initializer_list<Tensor*> inputs) {
        ...
        for (Tensor* input : inputs) {
            ...
            if (node->requires_grad) requires_grad = true;
        }
...
Tensor Tensor::operator/(const Tensor &rhs) const {
    // 验证形状和类型匹配
    if (_shape != rhs._shape) throw std::runtime_error("Shape mismatch in division");
    if (_dtype != rhs._dtype) throw std::runtime_error("DType mismatch in division");
...
Tensor Tensor::operator-(const Tensor &rhs) const {
    // 验证形状和类型匹配
    if (_shape != rhs._shape) throw std::runtime_error("Shape mismatch in subtraction");
    if (_dtype != rhs._dtype) throw std::runtime_error("DType mismatch in subtraction");
...
Tensor Tensor::operator*(const Tensor &rhs) const {
    // 验证形状和类型匹配
    if (_shape != rhs._shape) throw std::runtime_error("Shape mismatch in multiplication");
    if (_dtype != rhs._dtype) throw std::runtime_error("DType mismatch in multiplication");
