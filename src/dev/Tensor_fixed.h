...
public:
    ...
    Storage(size_t size, DType dtype, DeviceType device = DeviceType::kCPU): _size(size), _dtype(dtype), _device(device),_data(size > 0 ? std::shared_ptr<char[]>(new char[size * dtypeSize(dtype)]) : nullptr) {
        std::cout << ">>> Storage构造, 大小: " << size << ", 类型: " << dtypeToString(dtype) << std::endl;
    }
   ...
   ~Storage() = default;
    ...
    T* data() {
        if (_size == 0 || !_data) {
            std::cout << "!!! Storage::data() - 空存储" << std::endl;
            return nullptr;
        }
        checkDType<T>();
        T* result = reinterpret_cast<T*>(_data.get());
        std::cout << ">>> Storage::data() - 返回指针: " << static_cast<void*>(result) << std::endl;
        return result;
    }
   ...
   // 创建存储的深拷贝
    ...
    Storage clone() const {
        std::cout << ">>> Storage::clone - 开始, 大小: " << _size << std::endl;
        Storage new_storage(_size, _dtype, _device);
        if (_size > 0 && _data) {
            std::cout << ">>> Storage::clone - 复制数据, 大小: " << _size * dtypeSize(_dtype) << " 字节" << std::endl;
            ...
            if (_dtype == DType::kFloat && _size > 0) {
                ...
                std::cout << ">>> Storage::clone - 验证: 源值=" << src[0] << ", 目标值=" << dst[0] << std::endl;
            }
        }
        std::cout << "<<< Storage::clone - 完成" << std::endl;
        return new_storage;
    }
    // 添加清空方法
    // 修复 clear 方法
...
// 修复所有构造函数
    ...
    Tensor(float value) : tensor_id_(global_tensor_id++), _shape({}),
        ...
        if (_storage.data<float>()) {
            *_storage.data<float>() = value;
            std::cout << ">>> 标量Tensor设置完成, 存储值: " << *_storage.data<float>() << std::endl;
        } else {
            std::cout << "!!! 错误: 无法分配存储" << std::endl;
        }
    }
    ...
    Tensor& operator=(const Tensor& other) {
        if (this != &other) {
            ...
            std::cout << ">>> Tensor拷贝赋值, 新ID: " << tensor_id_ << ", 原ID: " << other.tensor_id_ << std::endl;
        }
        return *this;
    }
    ...
    ~Tensor() {
        ...
        if (tensor_id_ != 0 && !record_committed_) {
            std::cout << ">>> Tensor析构, ID: " << tensor_id_ << std::endl;
            commit_pending_record();
        }
    }
   ...
   // 标量访问（0维张量）
    ...
    T& item() {
        ...
        if (_storage.empty()) {
            std::cout << "!!! Tensor::item() - 存储为空" << std::endl;
            static T default_value;
            return default_value;
        }
        T* data_ptr = _storage.data<T>();
        if (!data_ptr) {
            std::cout << "!!! Tensor::item() - 数据指针为空" << std::endl;
            static T default_value;
            return default_value;
        }
        std::cout << ">>> Tensor::item() - 返回值: " << *data_ptr << std::endl;
        return *data_ptr;
    }
...
private:
    ...
    struct Node {
        ...
        void clear_grad_safely() {
            std::cout << ">>> Node::clear_grad_safely - 开始, 节点ID: " << tensor_id << std::endl;
            if (!grad.empty()) {
                grad.clear_storage();
            }
            std::cout << "<<< Node::clear_grad_safely - 完成" << std::endl;
        }
    };
...
public:
    void debug_print_state(const std::string& context) {
        ...
        std::cout << "=== AutoDiff状态 [" << context << "] ===" << std::endl;
        std::cout << "计算图节点 (" << id_to_node.size() << "): ";
        ...
        std::cout << "待处理记录 (" << pending_records.size() << "): ";
...
Tensor get_grad(const Tensor* t) {
    if (!t || t->id() == 0) {
        std::cout << ">>> get_grad: 无效输入" << std::endl;
        return Tensor();
    }

    std::cout << ">>> get_grad - 开始, 目标ID: " << t->id() << std::endl;
...

    {
        ...
        if (it != id_to_node.end() && it->second && it->second->requires_grad && !it->second->grad.empty()) {
            ...
            std::cout << ">>> 找到梯度，形状: [";
            for (auto s : grad_shape) std::cout << s << " ";
            std::cout << "], 值: " << grad_value << std::endl;
        }
    } // 释放锁

    // 第二步：在锁外创建结果Tensor
    if (has_grad) {
        std::cout << ">>> 创建梯度副本" << std::endl;
            ...
            if (it != id_to_node.end() && it->second && !it->second->grad.empty()) {
                ...
                switch (grad_dtype) {
                    case DType::kFloat: {
                        ...
                        std::cout << ">>> 复制了 " << count << " 个float值，第一个值: " << dst[0] << std::endl;
                        break;
                    }
                    case DType::kDouble: {
        ...
        std::cout << ">>> 最终梯度副本值: " << result.item<float>() << std::endl;
        std::cout << "<<< get_grad - 完成" << std::endl;
        return result;
    }

    std::cout << "<<< get_grad - 未找到梯度" << std::endl;
    return Tensor();
}
    ...
    void make_leaf(Tensor& t, bool requires_grad) {
        size_t id = t.id();
        if (id == 0) {
            std::cout << "!!! 错误: 尝试注册ID为0的张量" << std::endl;
            return;
        }

        std::cout << ">>> AutoDiff::make_leaf - 开始, ID: " << id << std::endl;
        debug_print_state("make_leaf开始前");

        {
            std::lock_guard<std::mutex> lock(records_mutex);
            if (id_to_node.find(id) != id_to_node.end()) {
                std::cout << ">>> 节点 " << id << " 已存在，跳过创建" << std::endl;
                return;
            }
        }
        ...
        std::cout << "<<< AutoDiff::make_leaf - 完成, ID: " << id << std::endl;
        debug_print_state("make_leaf完成后");
    }
    // 在 AutoDiff 类中添加
    ~AutoDiff() {
        std::cout << ">>> AutoDiff 析构" << std::endl;
        // 避免在析构时进行复杂操作
        // 直接清空，不持有锁
        id_to_node.clear();
