...
static mlir::OwningOpRef<mlir::ModuleOp> buildTensorMLIRModule(mlir::MLIRContext& context, const Graph& graph,
    ...
    // 辅助函数：根据算子输出获取其张量类型 (静态/动态)
    auto getTensorTypeForNode = [&](size_t node_id) {
        if (!use_static_shape) {
    ...
    for (size_t i = 0; i < num_inputs; ++i) {
        arg_types.push_back(getTensorTypeForNode(graph_inputs[i]));
    }
    for (size_t i = 0; i < num_outputs; ++i) {
        arg_types.push_back(getTensorTypeForNode(graph_outputs[i]));
    }

    std::vector<mlir::Type> ret_types;
    ret_types.reserve(num_outputs);
    for (size_t i = 0; i < num_outputs; ++i) {
        ret_types.push_back(getTensorTypeForNode(graph_outputs[i]));
    }
    ...
    for (size_t i = 0; i < num_inputs; ++i) {
        val_map[graph_inputs[i]] = entry->getArgument(static_cast<unsigned>(i));
    }
    ...
    for (const auto* node : compute_nodes) {
        ...
        if (it != graph_outputs.end()) {
            ...
            dest = entry->getArgument(static_cast<unsigned>(num_inputs + out_idx));
        } else {
            dest = empty_tensor; // 中间临时节点用 empty tensor
        }

        mlir::Value result;
        auto tensorType = getTensorTypeForNode(node->id);
    ...
    for (size_t out_id : graph_outputs) {
        returns.push_back(val_map.at(out_id));
    }
...
static void applyUnifiedTransformPipeline(mlir::ModuleOp module, size_t num_inputs, size_t num_outputs, size_t base_numel) {
    ...
    // 1. C3-to-Linalg Lowering
    {
        ...
        patterns.add<AddTensorOpLowering, SubTensorOpLowering, MulTensorOpLowering, DivTensorOpLowering,
                     ...
                     ExpTensorOpLowering, LogTensorOpLowering, GtTensorOpLowering, ConstTensorOpLowering>(module.getContext());
    ...
    // 6. Linalg to Loops & SCF/Arith/Math to LLVM
    {
        ...
        if (tile_size == 0 && base_numel >= 65536) {
            tile_size = 2048;
        }
...
LinalgOneShotKernel::LinalgOneShotKernel(const Graph& graph, int opt_level)
    ...
    impl_->context.loadDialect<mlir::tensor::TensorDialect>();
    impl_->context.loadDialect<mlir::linalg::LinalgDialect>();
    impl_->context.loadDialect<mlir::c3::C3Dialect>();
