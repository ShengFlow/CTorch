...
int main() {
    ...
    CtorchError::info(ErrorPlatform::kCPU,"激活函数测试完毕");

    // 测试损失函数
    test_loss_functions();
    CtorchError::info(ErrorPlatform::kCPU,"损失函数测试完毕");

    // 测试张量操作
    test_tensor_operations();
    CtorchError::info(ErrorPlatform::kCPU,"张量操作测试完毕");

    // 测试随机数生成
    test_random_tensors();
    CtorchError::info(ErrorPlatform::kCPU,"随机数测试完毕");

    CtorchError::info(ErrorPlatform::kCPU,"所有测试完成！");
    return 0;
}
