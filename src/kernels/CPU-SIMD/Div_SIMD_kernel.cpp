...
Tensor Div_SIMD_kernel(const Tensor& a, const Tensor& b) {
    ...
    if (a.sizes() != b.sizes()) {
        if (a.dim() == 0) {
            ...
            for (size_t i = 0; i < max_dims; ++i) {
                ...
                if (a_dim != 1 && b_dim != 1 && a_dim != b_dim) {
                    CtorchError::log(ErrorLevel::ERROR, ErrorPlatform::kCPU, ErrorType::DIMENSION,
                                     "CPU-SIMD Div_Kernel: Tensor形状不兼容，无法广播");
                    return Tensor();
                }
            ...
            return Div_SIMD_kernel(a_broadcasted, b_broadcasted);
        }
    }
    ...
    for (; i + 7 < elem_count; i += 8) {
        ...
        __m256 res_vec = _mm256_div_ps(a_vec, b_vec);
    ...
    for (; i + 3 < elem_count; i += 4) {
        ...
        float32x4_t res_vec = vdivq_f32(a_vec, b_vec);
