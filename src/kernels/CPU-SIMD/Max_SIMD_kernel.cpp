/**
 * @file Max_SIMD_kernel.cpp
 * @brief CPU-SIMD Max算子（逐元素最大值，支持广播）
 * @author GhostFace
 * @date 2026/06/30
 */
...
CT_HOT Tensor Max_SIMD_kernel(const Tensor& a, const Tensor& b) {
    ...
    if (a.sizes() == b.sizes()) {
        ...
        for (; i + 7 < count; i += 8) {
            ...
            _mm256_storeu_ps(&result_data[i], _mm256_max_ps(a_vec, b_vec));
        }
        for (; i < count; ++i) result_data[i] = std::max(a_data[i], b_data[i]);
#elif defined(__aarch64__)
        size_t i = 0;
        for (; i + 3 < count; i += 4) {
            float32x4_t a_vec = vld1q_f32(&a_data[i]);
            float32x4_t b_vec = vld1q_f32(&b_data[i]);
            vst1q_f32(&result_data[i], vmaxq_f32(a_vec, b_vec));
        }
        for (; i < count; ++i) result_data[i] = std::max(a_data[i], b_data[i]);
#else
        for (size_t i = 0; i < count; ++i) result_data[i] = std::max(a_data[i], b_data[i]);
#endif
        return result;
    }
    ...
    for (size_t i = 0; i < max_dims; ++i) {
        ...
        if (a_dim != 1 && b_dim != 1 && a_dim != b_dim) {
            CtorchError::throwException(ErrorPlatform::kCPU, ErrorType::DIMENSION,
                "CPU-SIMD Max_Kernel: Tensor形状不兼容");
        }
    ...
    if (max_dims == 2) {
        ...
        for (size_t i = 0; i < rows; ++i) {
            ...
            for (; j + 3 < cols; j += 4) {
                ...
                vst1q_f32(&result_data[i * cols + j], vmaxq_f32(a_vec, b_vec));
            }
            for (; j < cols; ++j) {
                size_t a_idx = a_row_offset + j * a_strides[1];
                size_t b_idx = b_row_offset + j * b_strides[1];
                result_data[i * cols + j] = std::max(a_data[a_idx], b_data[b_idx]);
            }
        }
#else
        for (size_t i = 0; i < rows; ++i) {
        ...
        for (size_t i = 0; i < elem_count; ++i) {
            ...
            result_data[i] = std::max(a_data[a_idx], b_data[b_idx]);
        }
    }
