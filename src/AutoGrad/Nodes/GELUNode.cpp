...
inline float gelu_derivative_scalar(float x) {
    ...
    float term2 = 0.5f * x * (1.0f - tanh_v * tanh_v) * kSqrt2OverPi * (1.0f + kGeluCoeffDeriv * x * x);
    return term1 + term2;
}
}

GELUNode::GELUNode(const std::vector<std::shared_ptr<Node>>& upStreamNodes, const std::vector<Tensor>& inputs)
