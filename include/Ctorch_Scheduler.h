 ...
 * @details 采用单例模式实现，负责管理所有 kernel
 * 映射关系，根据算子类型和设备类型， 自动查找并调用对应的 kernel，实现 kernel
 * 的统一调度。
 * @author GhostFace
 * @date 2025/12/20
 */
...
private:
  ...
  std::unordered_map<op, std::unordered_map<DeviceType, BinaryKernelFunc>>
      binary_kernel_map_;
  // 单输入算子映射表
  std::unordered_map<op, std::unordered_map<DeviceType, UnaryKernelFunc>>
      unary_kernel_map_;
...
public:
  ...
  Tensor dispatch(const Tensor &a, const Tensor &b, op op_type) {
    ...
    if (a.dtype() != b.dtype()) {
      Ctorch_Error::log(ErrorLevel::ERROR, ErrorPlatform::kGENERAL,
                        ErrorType::DATATYPE,
                        "Ctorch_Scheduler: Tensor类型不一致");
    }
    // 对于加法、乘法、减法、除法、交叉熵和矩阵乘法操作，允许形状不同（支持广播、不同标签格式和矩阵乘法维度要求）
    if (op_type != op::Add && op_type != op::Mul && op_type != op::Sub &&
        op_type != op::Div && op_type != op::CE && op_type != op::MatMul &&
        a.sizes() != b.sizes()) {
      Ctorch_Error::log(ErrorLevel::ERROR, ErrorPlatform::kGENERAL,
                        ErrorType::DIMENSION,
                        "Ctorch_Scheduler: Tensor形状不一致");
    }
