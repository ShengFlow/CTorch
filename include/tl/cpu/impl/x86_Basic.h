...
/* ************************************************************************** */
#ifdef HAS_AVX512DQ
template <typename T, TL_IF(T::N <= 8)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
  /* we do not guarantee that padded elements are zero */
  uint32_t x = value ? 0xffffffffu : 0x00;
  return _cvtu32_mask8(x);
}
template <typename T, TL_IF(T::N == 16)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
  uint32_t x = value ? 0xffffffffu : 0x00;
  return _cvtu32_mask16(x);
}
template <typename T, TL_IF(T::N == 32)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
  uint32_t x = value ? 0xffffffffu : 0x00;
  return _cvtu32_mask32(x);
}
template <typename T, TL_IF(T::N == 64)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
...
template <typename T, TL_IF(T::N <= 8)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::N <= 8)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilege(T t, nint_t a, nint_t b) {
  nint_t end = std::clamp<nint_t>(b - a, 0, 8);
  return _cvtu32_mask8(~tailing_mask(int32_t(end)));
}
template <typename T, TL_IF(T::N == 16)>
...
#else // HAS_AVX512DQ
template <typename T, TL_IF(T::Bytes <= 16)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
  /* we do not guarantee that padded elements are zero */
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(sizeof(TypeOf<T>) == 1)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(sizeof(TypeOf<T>) == 4)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(sizeof(TypeOf<T>) == 8)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
#if VEC_WIDTH >= 256
template <typename T, TL_IF(T::Bytes == 32)>
CT_ALWAYS_FORCEINLINE Mask<T> mfill(T t, bool value) {
  /* we do not guarantee that padded elements are zero */
  int64_t x = value ? int64_t(-1) : 0;
...
template <typename T, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 1)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 4)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
template <typename T, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 8)>
CT_ALWAYS_FORCEINLINE Mask<T> mwhilelt(T t, nint_t a, nint_t b) {
...
/* ************************************************************************** */
#ifdef HAS_AVX512DQ
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M bit_and(M a, M b) {
  return _kand_mask8(a.v, b.v);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N == 16)>
CT_ALWAYS_FORCEINLINE M bit_and(M a, M b) {
  return _kand_mask16(a.v, b.v);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N == 32)>
CT_ALWAYS_FORCEINLINE M bit_and(M a, M b) {
  return _kand_mask32(a.v, b.v);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N == 64)>
CT_ALWAYS_FORCEINLINE M bit_and(M a, M b) {
  return _kand_mask64(a.v, b.v);
}

template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M bit_or(M a, M b) {
...
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M bit_xor(M a, M b) {
...
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M bit_andnot(M a, M b) {
...
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M bit_not(M a, M b) {
...
#else // HAS_AVX512DQ
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::Bytes <= 16)>
CT_ALWAYS_FORCEINLINE M bit_and(M a, M b) {
...
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::Bytes <= 16)>
CT_ALWAYS_FORCEINLINE M bit_not(M m) {
...
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, bfloat16_t, float16_t>)>
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, bfloat16_t, float16_t>)>
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, bfloat16_t, float16_t>)>
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_none<TypeOf<T>, float32_t, float64_t>)>
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_none<TypeOf<T>, float32_t, float64_t>)>
CT_ALWAYS_FORCEINLINE V blend(V v0, Mask<T> m, V v1) {
  // assuming that mask is sanitized (all bits corresponding to active element are 1)
...
template <int I7, int I6, int I5, int I4, int I3, int I2, int I1, int I0, typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 16), TL_IF(sizeof(TypeOf<T>) == 2)>
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 16), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
  Tag<int8_t, 16> t1;
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 16), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v, int i7, int i6, int i5, int i4, int i3, int i2, int i1, int i0) {
  return word::local_shuf(v, _mm_set_epi16(i7, i6, i5, i4, i3, i2, i1, i0));
}

#if VEC_WIDTH >= 256
template <int I7, int I6, int I5, int I4, int I3, int I2, int I1, int I0, typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v) {
  if constexpr (I3 < 4 && I2 < 4 && I1 < 4 && I0 < 4 &&
                I7 >= 4 && I6 >= 4 && I5 >= 4 && I4 >= 4) {
    // Fast path: low 4 elements all from [0,3], high 4 all from [4,7]
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
  Tag<int8_t, 32> t1;
  // we assume that high byte of each index in i are zero
...
#if VEC_WIDTH >= 512
template <int I7, int I6, int I5, int I4, int I3, int I2, int I1, int I0, typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v) {
  if constexpr (I3 < 4 && I2 < 4 && I1 < 4 && I0 < 4 &&
                I7 >= 4 && I6 >= 4 && I5 >= 4 && I4 >= 4) {
    // Fast path: low 4 elements all from [0,3], high 4 all from [4,7]
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(sizeof(TypeOf<T>) == 2)>
CT_ALWAYS_FORCEINLINE V local_shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
  Tag<int8_t, 64> t1;
  // we assume that high byte of each index in i are zero
...
template <int I1, int I0, typename V, typename T = Vec2Tag<V>, TL_IF(sizeof(V) == 32), TL_IF(is_none<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE V block_shuf(V v) {
  Tag<float32_t, 8> t1;
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(sizeof(V) == 32), TL_IF(is_none<TypeOf<T>, float32_t, float64_t, int32_t>)>
...
#if VEC_WIDTH >= 256
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE V shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE V shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
  #ifdef HAS_AVX512BW
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE V shuf(V v, Vec<Rebind<Index<TypeOf<T>>, T>> i) {
...
//                                Constructors                                //
/* ************************************************************************** */
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_ps(v);
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_pd(v);
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_epi64x((long long)(v));
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_epi32(int32_t(v));
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_epi16(int16_t(v));
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
return _mm_set1_epi8(int8_t(v));
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
union { bfloat16_t b; int16_t i; } u { .b = v };
return _mm_set1_epi16(u.i);
}
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
#ifdef HAS_AVX512_FP16
...
#if VEC_WIDTH >= 256
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_ps(v);
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_pd(v);
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_epi64x((long long)(v));
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_epi32(int32_t(v));
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_epi16(int16_t(v));
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm256_set1_epi8(int8_t(v));
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  union { bfloat16_t b; int16_t i; } u { .b = v };
  return _mm256_set1_epi16(u.i);
}
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  #ifdef HAS_AVX512_FP16
  return _mm256_castph_si256(_mm256_set1_ph(v));
  #else
...
#if VEC_WIDTH >= 512
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_ps(v);
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_pd(v);
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_epi64((long long)(v));
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_epi32(int32_t(v));
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_epi16(int16_t(v));
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  return _mm512_set1_epi8(int8_t(v));
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  union { bfloat16_t b; int16_t i; } u { .b = v };
  return _mm512_set1_epi16(u.i);
}
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, float16_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v) {
  #ifdef HAS_AVX512_FP16
 ...
 */
template <typename T>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v, Mask<T> m, Vec<T> default_v) {
return word::blend(default_v, m, word::fill(t, v));
}
template <typename T>
CT_ALWAYS_FORCEINLINE Vec<T> fill(T t, TypeOf<T> v, nint_t n, Vec<T> default_v) {
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> zeros(T t) {
...
template <typename T, TL_IF(T::Bytes <= 16), TL_IF(is_none<TypeOf<T>, float32_t, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> zeros(T t) {
...
template <typename T, TL_IF(T::Bytes == 32), TL_IF(is_none<TypeOf<T>, float32_t, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> zeros(T t) {
...
template <typename T, TL_IF(T::Bytes == 64), TL_IF(is_none<TypeOf<T>, float32_t, float64_t>)>
CT_ALWAYS_FORCEINLINE Vec<T> zeros(T t) {
...
/* ************************************************************************** */
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE TypeOf<T> get(V v, nint_t i) {
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t, int16_t, uint16_t, int8_t, uint8_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE TypeOf<T> get(V v, nint_t i) {
  Rebind<Index<TypeOf<T>>, T> ti;
  auto u = word::shuf(v, Vec<decltype(ti)>{_mm_cvtsi32_si128(i)});
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t, int16_t, uint16_t, int8_t, uint8_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE TypeOf<T> get(V v, nint_t i) {
  Rebind<Index<TypeOf<T>>, T> ti; ViewAs<Index<TypeOf<T>>, T> th;
  auto u = word::shuf(v, word::bitcast(ti, Vec<decltype(th)>{_mm256_castsi128_si256(_mm_cvtsi32_si128(i))}));
...
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t, int16_t, uint16_t, int8_t, uint8_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE TypeOf<T> get(V v, nint_t i) {
  Rebind<Index<TypeOf<T>>, T> ti; ViewAs<Index<TypeOf<T>>, T> th;
  auto u = word::shuf(v, word::bitcast(ti, Vec<decltype(th)>{_mm512_castsi128_si512(_mm_cvtsi64_si128(i))}));
...
#ifdef HAS_AVX512DQ
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm_mask_mov_ps(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm_mask_mov_pd(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask16(1u << (unsigned) i);
  return _mm_mask_mov_epi8(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm_mask_mov_epi16(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm_mask_mov_epi32(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm_mask_mov_epi64(v.v, m, fill(T(), x).v);
}

template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm256_mask_mov_ps(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm256_mask_mov_pd(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask32(1u << (unsigned) i);
  return _mm256_mask_mov_epi8(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask16(1u << (unsigned) i);
  return _mm256_mask_mov_epi16(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm256_mask_mov_epi32(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm256_mask_mov_epi64(v.v, m, fill(T(), x).v);
}

template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, float32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask16(1u << (unsigned) i);
  return _mm512_mask_mov_ps(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, float64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm512_mask_mov_pd(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int8_t, uint8_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu64_mask64(1uLL << (unsigned) i);
  return _mm512_mask_mov_epi8(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int16_t, uint16_t, float16_t, bfloat16_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask32(1u << (unsigned) i);
  return _mm512_mask_mov_epi16(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask16(1u << (unsigned) i);
  return _mm512_mask_mov_epi32(v.v, m, fill(T(), x).v);
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 64), TL_IF(is_any<TypeOf<T>, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  auto m = _cvtu32_mask8(1u << (unsigned) i);
  return _mm512_mask_mov_epi64(v.v, m, fill(T(), x).v);
}
#else // HAS_AVX512DQ
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float32_t, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  static const __m128i idx = _mm_setr_epi32(0, 1, 2, 3);
  auto m = _mm_cmpeq_epi32(_mm_set1_epi32(i), idx);
  return word::blend(v, Mask<T>{m}, fill(T(), x));
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes <= 16), TL_IF(is_any<TypeOf<T>, float64_t, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  static const __m128i idx = _mm_setr_epi32(0, 0, 1, 1);
  auto m = _mm_cmpeq_epi32(_mm_set1_epi32(i), idx);
  return word::blend(v, Mask<T>{m}, fill(T(), x));
...
#if VEC_WIDTH >= 256
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float32_t, int32_t, uint32_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  static const __m256i idx = _mm256_setr_epi32(0, 1, 2, 3, 4, 5, 6, 7);
  auto m = _mm256_cmpeq_epi32(_mm256_set1_epi32(i), idx);
  return word::blend(v, Mask<T>{m}, fill(T(), x));
}
template <typename V, typename T = Vec2Tag<V>, TL_IF(T::Bytes == 32), TL_IF(is_any<TypeOf<T>, float64_t, int64_t, uint64_t>)>
CT_ALWAYS_FORCEINLINE V set(V v, nint_t i, TypeOf<T> x) {
  static const __m256i idx = _mm256_setr_epi32(0, 0, 1, 1, 2, 2, 3, 3);
  auto m = _mm256_cmpeq_epi32(_mm256_set1_epi32(i), idx);
  return word::blend(v, Mask<T>{m}, fill(T(), x));
...
#ifdef HAS_AVX512DQ
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE bool get(M m, nint_t i) {
...
#else // HAS_AVX512DQ
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::ElSize == 1)>
CT_ALWAYS_FORCEINLINE bool get(M m, nint_t i) {
  Tag<int8_t, M::N> t;
  return !!get(Vec<decltype(t)>{m.v}, i);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::ElSize == 2)>
CT_ALWAYS_FORCEINLINE bool get(M m, nint_t i) {
  Tag<int16_t, M::N> t;
  return !!get(Vec<decltype(t)>{m.v}, i);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::ElSize == 4)>
CT_ALWAYS_FORCEINLINE bool get(M m, nint_t i) {
  Tag<int32_t, M::N> t;
  return !!get(Vec<decltype(t)>{m.v}, i);
}
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::ElSize == 8)>
CT_ALWAYS_FORCEINLINE bool get(M m, nint_t i) {
  Tag<int64_t, M::N> t;
  return !!get(Vec<decltype(t)>{m.v}, i);
}
...
#ifdef HAS_AVX512DQ
template <typename M, TL_IF(x86::is_reg_mask<M>), TL_IF(M::N <= 8)>
CT_ALWAYS_FORCEINLINE M set(M m, nint_t i, bool x) {
