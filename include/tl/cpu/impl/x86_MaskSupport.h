...
TLV_INLINE __m128i mask8_to_epi16_simd(uint8_t mask) {
...
TLV_INLINE uint8_t epi16_to_mask8_simd(__m128i vec) {
  static const __m128i bits = _mm_set_epi16(0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01);
...
TLV_INLINE __m128i mask16_to_epi8_simd(uint16_t mask) {
  static const __m128i bits = _mm_set_epi16(0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01);
...
TLV_INLINE __m128i mask4_to_epi32_simd(uint8_t mask) {
...
TLV_INLINE __m128i mask8_to_epi16_bmi2(uint8_t mask) {
...
TLV_INLINE __m128i mask4_to_epi32_bmi2(uint8_t mask) {
...
TLV_INLINE __m128i mask8_to_epi16(uint8_t mask) {
  return mask8_to_epi16_simd(mask);
}

// 8个16位元素 (MSB) → 8位mask
TLV_INLINE uint8_t epi16_to_mask8(__m128i vec) {
  #ifdef HAS_BMI2
...
// 16位mask → 16个8位元素
TLV_INLINE __m128i mask16_to_epi8(uint16_t mask) {
  #ifdef HAS_BMI2
...
// 16个8位元素 → 16位mask
TLV_INLINE uint16_t epi8_to_mask16(__m128i vec) {
  return (uint16_t)_mm_movemask_epi8(vec);
}

// 4位mask → 4个32位元素
TLV_INLINE __m128i mask4_to_epi32(uint8_t mask) {
  return mask4_to_epi32_simd(mask);
}

// 4个32位元素 → 4位mask
TLV_INLINE uint8_t epi32_to_mask4(__m128i vec) {
