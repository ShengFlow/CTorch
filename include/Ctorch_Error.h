...
private:
   ...
   * @brief 打印欢迎信息
   * @details
   * 打印Ctorch的欢迎界面，包括版本信息、CUDA信息、C++标准、编译器信息、构建时间和系统信息
   */
  static void welCome() {
    ...
    printf("$$ |  $$\\    $$ |   $$ |  $$ |$$ |  $$ |$$ |  $$\\ $$ |  $$ |\n");
    printf("\\$$$$$$  |   $$ |    $$$$$$  |$$ |  $$ |\\$$$$$$  |$$ |  $$ |\n");
    printf(" \\______/    \\__|    \\______/ \\__|  \\__| \\______/ \\__|  "
           "\\__|\n");
    printf("============================================================\n");
...
public:
  ...
  static Ctorch_Stats &getInstance() {
    static Ctorch_Stats
        instance_; // 这里利用了一个巧妙的C++特性，保证全局只会实例化一个instance_
 ...
 * @brief Ctorch错误处理类
 * @details
 * 用于记录和管理CTorch中的各种错误信息，包括日志记录、错误统计和异常抛出
 */
class Ctorch_Error {
  /**
  ...
  static std::string getPlatformName(ErrorPlatform platform) {
    switch (platform) {
    case ErrorPlatform::kUNKNOWN:
      return "UNKNOWN";
    case ErrorPlatform::kCPU:
      return "CPU";
    case ErrorPlatform::kCUDA:
      return "CUDA";
    case ErrorPlatform::kMPS:
      return "MPS";
    case ErrorPlatform::kAMX:
      return "AMX";
    case ErrorPlatform::kGENERAL:
      return "GENERAL";
    case ErrorPlatform::kAutoDiff:
      return "AUTO_DIFF";
    default:
      return "UNKNOWN";
    }
  ...
  static std::string getTypeName(ErrorType type) {
    ...
    case ErrorType::MEMORY:
      return "MEMORY";
    case ErrorType::DIMENSION:
      return "DIMENSION";
    case ErrorType::DEVICE_COMPAT:
      return "DEVICE_COMPAT";
    case ErrorType::DATATYPE:
      return "DATATYPE";
    case ErrorType::KERNEL_LAUNCH:
      return "KERNEL_LAUNCH";
    case ErrorType::TENSOR_STATE:
      return "TENSOR_STATE";
    case ErrorType::PLATFORM_API:
      return "PLATFORM_API";
    default:
      return "UNKNOWN";
    }
  ...
  static std::string getPrintLevelName(PrintLevel level) {
    switch (level) {
    case PrintLevel::MINIUM:
      return "MINIUM";
    case PrintLevel::MEDIUM:
      return "MEDIUM";
    case PrintLevel::FULL:
      return "FULL";
    default:
      return "UNKNOWN";
    }
...
public:
   ...
   * @param msg 错误信息
   */
  static void log(ErrorLevel level, ErrorPlatform platform, ErrorType type,
                  const std::string msg) {
    Ctorch_Stats &inst = Ctorch_Stats::getInstance();
    if (inst.level == PrintLevel::MEDIUM && level == ErrorLevel::TRACE)
      return;
    if (inst.level == PrintLevel::MINIUM &&
        (level == ErrorLevel::DEBUG || level == ErrorLevel::TRACE))
      return;
    ...
    printf("[%s] [%s %" PRIu64 "] [ERROR_CODE:0x%" PRIX32
           "] [PLATFORM:%s] [TYPE:%s] %s\n",
           getLevelName(level).c_str(), getFormattedTimeMs().c_str(),
           getTimestampMs(), error_code, getPlatformName(platform).c_str(),
           getTypeName(type).c_str(), msg.c_str());
    printf(ESC_END);
  ...
  static void info(ErrorPlatform platform, const std::string &msg) {
    ...
    printf("[INFO]" ESC_END "  [%s %" PRIu64 "] [PLATFORM:%s] %s\n",
           ...
           getPlatformName(platform).c_str(), msg.c_str());
    printf(ESC_END);
  }
  ...
  static void stats() {
    ...
    printf(ESC_START COLOR_INFO "[INFO]  " ESC_END "Total FATAL: %" PRIu64 "\n",
           inst.getTotalFatal());
    printf(ESC_START COLOR_INFO "[INFO]  " ESC_END "Total Time: %" PRIu64
                                "ms\n",
  ...
  static void trace(ErrorPlatform platform, std::string msg) {
    Ctorch_Stats &inst = Ctorch_Stats::getInstance();
    if (inst.level == PrintLevel::MINIUM || inst.level == PrintLevel::MEDIUM)
      return;
    printf(ESC_START COLOR_DEBUG);
   ...
   * @param msg 错误信息
   */
  static void error(ErrorPlatform platform, ErrorType type,
                    const std::string &msg) {
   ...
   * @param msg 错误信息
   */
  static void warn(ErrorPlatform platform, ErrorType type,
                   const std::string &msg) {
   ...
   * @param msg 错误信息
   */
  static void fatal(ErrorPlatform platform, ErrorType type,
                    const std::string &msg) {
   ...
   */
  static void throwException(ErrorPlatform platform, ErrorType type,
                             const std::string &msg) {
    // 首先记录错误
    log(ErrorLevel::ERROR, platform, type, msg);
    // 然后抛出异常
   ...
   * @throw std::runtime_error 抛出运行时异常
   */
  static void throwFatalException(ErrorPlatform platform, ErrorType type,
                                  const std::string &msg) {
    // 首先记录致命错误
    log(ErrorLevel::FATAL, platform, type, msg);
    // 然后抛出异常
