 ...
 * @version v3.1
 * @details
 * 定义了张量类(Tensor)及其相关操作，包括存储管理、自动微分、各种数学运算等。
 */
#ifndef TENSOR_H
#define TENSOR_H
...
#include <vector>
// #include<omp.h>
// !!!目前不确定在哪些机器上需要这个头文件，如果编译错误，可以尝试加上
// ======================= 前向声明 =======================
class Tensor;
class Storage;
// ======================= 存储类 (Storage) =======================
...
private:
  ...
  void elementwiseOp(Tensor &result, const Tensor &a, const Tensor &b,
                     Op op) const;
   ...
   */
  template <typename T, typename Op>
  void broadcast_elementwise_op(Tensor &result, const Tensor &a,
                                const Tensor &b, const BroadCastResult &bc,
                                Op op) const;
   ...
   */
  template <typename T>
  void printRecursive(std::ostream &os, size_t dim,
...
public:
   ...
   */
  Tensor(float value)
      : tensor_id_(global_tensor_id++), _storage_offset(0),
   ...
   */
  Tensor(ShapeTag /*tag*/, const std::vector<size_t> &shape,
         DType dtype = DType::kFloat, DeviceType device = DeviceType::kCPU,
         bool zero_init = true)
      : tensor_id_(global_tensor_id++), _storage_offset(0), _device(device),
        _dtype(dtype) {
    _shape = shape;
    computeStrides();
  ...
  Tensor(const Tensor &other)
      : tensor_id_(global_tensor_id++), record_committed_(false),
        ...
        _storage_offset(other._storage_offset), _device(other._device),
        _dtype(other._dtype),
        _storage(other._storage.clone()), // 注意：这里调用了clone()
        _shape(other._shape) {
    // std::cout << ">>> Tensor拷贝构造, 新ID: " << tensor_id_ << ", 原ID: " <<
    // other.tensor_id_ << std::endl;
    std::ostringstream oss;
    oss << ">>> Tensor拷贝构造, 新ID: " << tensor_id_
        << ", 原ID: " << other.tensor_id_;
    std::string msg = oss.str();
    Ctorch_Error::trace(ErrorPlatform::kCPU, msg);
  }
   ...
   */
  [[nodiscard]] bool
  check_index_bounds(const std::vector<size_t> &indices) const;
