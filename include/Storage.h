 ...
 * @details
 * 存储类支持多种数据类型和设备类型，使用shared_ptr实现共享所有权，减少内存占用
 */
class Storage {
private:
  /**
   ...
   * @brief 原始内存指针
   * @details
   * 使用shared_ptr<char[]>实现共享所有权，避免手动delete问题和数组delete不匹配问题
...
public:
   ...
   */
  template <typename T>
  Storage(const T *data, size_t size, DType dtype,
