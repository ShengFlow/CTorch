...
void Tensor::checkDType() const {
    if ((std::is_same<T, float>::value && _dtype != DType::kFloat) ||
        ...
        std::cerr << "Tensor data type mismatch: T=" << typeid(T).name()
...
// 支持广播的逐元素操作
template<typename T, typename Op>
void Tensor::broadcast_elementwise_op(Tensor& result, const Tensor& a, const Tensor& b,
                                     const BroadCastResult& bc, Op op) const {
    // 这里需要实现支持广播的逐元素操作
    // 暂时使用简单的实现
    elementwiseOp<T>(result, a, b, op);
}
