...
class Module : public ModuleBase{
  private:
    ...
    using ForwardPreHook = HOOK_RET(*)(const Module* self,const std::vector<std::optional<Tensor>> input);
    using ForwardHook = HOOK_RET(*)(const Module* self,const std::vector<std::optional<Tensor>> grad_input, std::vector<std::optional<Tensor>> grad_output);
    using FullModuleBackwardHook = BACKWARD_HOOK_RET(*)(const Module& self,const std::vector<std::optional<Tensor>> grad_input, std::vector<std::optional<Tensor>> grad_output);
    ...
    static void load_tensor(std::ifstream& is, Tensor& tensor){
        ...
        is.read(reinterpret_cast<char*>(strides.data()),
                strides_size * sizeof(size_t));
  ...
  public:
    ...
    template <typename... Args>
    auto forward(Args&&... args){ return static_cast<Derived*>(this)->forward(std::forward<Args>(args)...);}
    ...
    auto operator()(Args... args){
        for (auto elem:args) {
            if (!(std::is_fundamental_v<decltype(elem)> && std::is_pointer(elem)) && std::is_same_v<decltype(elem),Tensor>) _argsGrad.push_back(std::optional<Tensor&>(elem.grad()));
        }
        for (auto fn:_forwardPreHooks)
            if (fn(this,args).has_value()) args = fn(this, args).value();
        auto result = forward(args);
        for (auto fn:_forwardHooks)
            if (fn(this,args,result).has_value()) result = fn(this, args, result).value();
        for (auto elem:result) {
            if (!(std::is_fundamental_v<decltype(elem)> && std::is_pointer(elem)) && std::is_same_v<decltype(elem),Tensor>) _outputsGrad.push_back(std::optional<Tensor&>(elem.grad()));
        }
        return result;
    }
    ...
    void train(bool recur){
        if (recur) {
            auto root      = this;
            auto recursive = [&recursive, this](Module *root) -> void {
                root->_train = true;
                std::unordered_map<std::string,Module*> children;
                for (auto [name,ptr]:_children) children.emplace(name,static_cast<Module*>(ptr.get()));
    ...
    void addChild(const std::string& name,std::unique_ptr<ModuleBase> child) {
        if (!child)
            if (!_children.contains(name)) _children.emplace(name,std::move(child));
            else throw std::runtime_error("Child "+name+" already exists");
        else throw std::invalid_argument("addChild");
    }
    ...
    std::unordered_map<std::string,Module*> children() const {
        ...
        for (auto [name,child]:_children) {
            ...
            if (!ch) result.emplace(name,child);
            else throw std::runtime_error("Unsupported child type "+name);
        }
        return result;
    }
    ...
    std::vector<Module*> childrenRecur(Module* root) const{
        ...
        auto recursive = [&result, &recursive, this](Module *re_root) -> void {
    ...
    void apply(Module* root,auto func,Args...args) {
        auto recursive = [&func, &recursive, &args](Module* root)->void {
            std::unordered_map<std::string,Module*> children;
            for (auto [name,ptr]:root->_children) children.emplace(name,static_cast<Module*>(ptr.get()));
    ...
    void registerParameter(const std::string& name,Parameter* parameter){
        if (parameter->isInitialized()) _parameters.emplace(name,std::make_unique<Parameter>(*parameter));
        else throw std::runtime_error("Parameter '"+name+"' is not initialized");
    }

    void registerParameters(std::unordered_map<std::string,std::unique_ptr<Parameter>>& parameters){
    ...
    [[nodiscard]] Parameter parameter(const std::string& name) const { return *_parameters.at(name); }

    [[nodiscard]] std::vector<Parameter> parameters(std::initializer_list<std::string> names) const{
    ...
    [[nodiscard]] std::vector<Buffer> buffers(const std::initializer_list<std::string> names) const{
    ...
    void registerFullModuleBackwardHook(FullModuleBackwardHook func){ _fullModuleBackwardHooks.push_back(func); }
    ...
    std::vector<FullModuleBackwardHook> fullModuleBackwardHooks() const{ return _fullModuleBackwardHooks; }
    ...
    // ======================= 公开IO =======================
    [[nodiscard]] std::vector<std::unordered_map<std::string,Tensor*>> state(std::string prefix,bool keepVars) const{
    ...
    void operator<<(Module* content) const{
        ...
        auto recursive = [&recursive](std::string name,Module* root)->void {
            std::unordered_map<std::string,Module*> children;
            for (auto [name,child]:root->_children) children.emplace(name,child);
        ...
        for (auto& [key,val]:_children) recursive(key,static_cast<Module*>(val.get()));
        std::cout<<"   )\n";
        std::cout<<"   Parameters(\n";

        for (const auto &key : _parameters | std::views::keys) std::cout<<"Parameter Name:"<<key<<std::endl;
        std::cout<<"   )\n";
        std::cout<<"   Buffers(\n";
        for (const auto &key : _buffers | std::views::keys) std::cout<<"Buffer Name:"<<key<<std::endl;
        std::cout<<"   )\n"<<")";
    }
    ...
    virtual void setExtraState(const std::vector<std::unordered_map<std::string,Tensor*>>& state){}

    [[nodiscard]] virtual std::vector<std::unordered_map<std::string,Tensor*>> getExtraState() const{ return {};}
};
