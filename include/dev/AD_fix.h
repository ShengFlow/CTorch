 ...
 private:
   ...
   // 支持广播的逐元素操作
    ...
    void broadcast_elementwise_op(Tensor& result, const Tensor& a, const Tensor& b,
       ...
       std::cout << ">>> 进入 broadcast_elementwise_op" << std::endl;
       ...
       for (size_t flat_idx = 0; flat_idx < total_elements; ++flat_idx) {
           if (flat_idx % 1000000 == 0) { // 每100万次输出一次进度
               std::cout << ">>> 处理元素: " << flat_idx << "/" << total_elements << std::endl;
           }
       ...
       std::cout << "<<< 离开 broadcast_elementwise_op" << std::endl;
   }
...
// 修复所有构造函数
    Tensor() : tensor_id_(global_tensor_id++), _storage_offset(0),
    ...
    Tensor(const Tensor& other)
        : tensor_id_(global_tensor_id++),
          ...
          record_committed_(false) {  // 新对象记录未提交
    }
   ...
   Tensor transpose() const {
    ...
    void debug_info(const std::string& name = "") const {
       ...
       std::cout << "], record_committed: " << (record_committed_ ? "true" : "false")
                 << std::endl;
   }
...
private:
    ...
    struct Node {
        ...
        Node(size_t id, Tensor t, bool req_grad, bool leaf = true)
            ...
            // 直接初始化grad，避免调用Tensor的方法
            if (requires_grad && !tensor.empty()) {
                // 直接构造梯度张量，不通过任何可能触发自动微分记录的方法
                grad = Tensor(ShapeTag{}, tensor.sizes(), tensor.dtype(), tensor.device(), true);
                grad.zero();
            }
        }
    };
...
public:
    ...
    Tensor get_grad(const Tensor* t) {
        if (!t || t->id() == 0) return Tensor();

        std::lock_guard<std::mutex> lock(records_mutex);
        auto it = id_to_node.find(t->id());
        if (it != id_to_node.end() && it->second && it->second->requires_grad) {
            return it->second->grad.clone(); // 返回副本
        }
        return Tensor();
    }
    ...
    void make_leaf(Tensor& t, bool requires_grad) {
        size_t id = t.id();
        if (id == 0) {
            std::cout << "警告: 尝试注册ID为0的张量" << std::endl;
            return;
        }
        ...
        std::cout << "<<< 叶子节点创建完成: " << id << std::endl;
    }

    // 删除旧的record_op函数，使用新的defer_record
    void defer_record(size_t output_id, op operation, const std::vector<Tensor*>& inputs) {
        std::cout << ">>> 进入 defer_record, output_id: " << output_id << std::endl;

        if (output_id == 0) {
            std::cout << "警告: output_id 为0" << std::endl;
            return;
        }
        ...
        for (Tensor* input : inputs) {
            if (input && input->id() != 0) {
                ...
                std::cout << ">>> 处理输入: " << input->id() << std::endl;
                ...
                if (needs_registration) {
                    std::cout << ">>> 注册叶子节点: " << input->id() << std::endl;
        ...
        // 现在持有锁并设置记录
        {
            ...
            std::cout << ">>> defer_record: 成功插入记录 " << output_id << std::endl;
            std::cout << ">>> 当前pending_records内容: ";
            for (const auto& pair : pending_records) {
                std::cout << pair.first << "(committed:" << pair.second.committed << ") ";
            }
            std::cout << std::endl;
        }

        std::cout << "<<< 离开 defer_record" << std::endl;
    }
    // 新增：提交延迟记录
    void commit_record(Tensor& output) {
    size_t output_id = output.id();
    if (output_id == 0) {
        std::cout << ">>> 警告: 尝试提交ID为0的张量" << std::endl;
        return;
    }
...

    {
        ...
        if (it == pending_records.end()) {
            std::cout << ">>> 错误: 找不到待处理记录 " << output_id << std::endl;
            return;
        }
    ...
    if (should_create_node) {
        ...
        // 设置梯度张量
        if (requires_grad) {
            output_node->grad = Tensor(ShapeTag{}, output.sizes(), output.dtype(), output.device());
            output_node->grad.zero();
        }
    ...
    Node* root_node = nullptr;
    {
        ...
        if (grad_output.empty()) {
            ...
            root_node->grad = grad_output.clone();
        }
    }
    ...
    // 阶段4：反向传播（每个节点独立处理）
    std::cout << ">>> 阶段4: 反向传播" << std::endl;
    for (size_t node_id : order) {
        std::cout << ">>> 处理节点: " << node_id << std::endl;
...
private:
    ...
    Tensor reduce_to_match(Tensor grad, const std::vector<size_t>& target_shape) {
        ...
        // 简化实现 - 对于加法操作，梯度通常不需要reduce
        // 因为加法的梯度传播就是直接传递梯度
        std::cout << ">>> 执行简化reduce" << std::endl;

        // 如果目标形状是标量，而梯度是向量，需要求和
        if (target_shape.empty() && !grad.shape().empty()) {
            Tensor result = grad.sum();
            std::cout << "<<< 离开 reduce_to_match (求和)" << std::endl;
            return result;
        }

        // 其他情况，暂时直接返回梯度（需要更复杂的实现）
        std::cout << "<<< 离开 reduce_to_match (直接返回)" << std::endl;
        return grad;
    }
};
...
Tensor apply_binary_operator(const Tensor& lhs, const Tensor& rhs,
    ...
    if (lhs.device() != rhs.device()) {
        throw std::runtime_error(std::string(op_name) + ": Device mismatch");
    }

    std::cout << ">>> 检查通过，准备广播..." << std::endl;

    // 广播处理
    BroadCastResult bc = broadCast(lhs, rhs);
    std::cout << ">>> 广播完成，创建结果张量..." << std::endl;

    Tensor result(ShapeTag{}, bc.logicShape, lhs.dtype(), lhs.device());
    std::cout << ">>> 结果张量创建完成，ID: " << result.id() << std::endl;
    ...
    // 梯度需求传播
    result.requires_grad(Tensor::should_require_grad(lhs, rhs));
    std::cout << ">>> 梯度需求设置完成: " << result.requires_grad() << std::endl;

    // 延迟记录操作
    AutoDiff* ctx = AutoDiffContext::current();
    if (ctx) {
        std::cout << ">>> 找到AutoDiff上下文，准备记录操作..." << std::endl;
        std::vector<Tensor*> inputs = {
            const_cast<Tensor*>(&lhs),
            const_cast<Tensor*>(&rhs)
        };
        ctx->defer_record(result.id(), operation_type, inputs);
        std::cout << ">>> 操作记录完成" << std::endl;
    } else {
        std::cout << ">>> 未找到AutoDiff上下文，跳过记录" << std::endl;
    }
    // 在返回结果前立即提交记录
    result.commit_pending_record();
    std::cout << ">>> 操作记录已提交，结果ID: " << result.id() << std::endl;
...
void Tensor::commit_pending_record() {
    if (!record_committed_ && tensor_id_ != 0) {
        AutoDiff* ctx = AutoDiffContext::current();
        if (ctx) {
            std::cout << ">>> 提交待处理记录: " << tensor_id_ << std::endl;
            ctx->commit_record(*this);
            record_committed_ = true;
            std::cout << "<<< 记录提交完成: " << tensor_id_ << std::endl;
        }
    }
}
